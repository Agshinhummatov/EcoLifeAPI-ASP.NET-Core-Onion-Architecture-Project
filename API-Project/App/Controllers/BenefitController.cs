...
namespace App.Controllers
{
    ...
    public class BenefitController : ControllerBase
    {
        ...
        [HttpGet]
        [ProducesResponseType(statusCode: StatusCodes.Status200OK, Type = typeof(IEnumerable<BenefitListDto>))]
        ...
        [HttpGet]
        [Route("{id}")]
        [ProducesResponseType(statusCode: StatusCodes.Status200OK, Type = typeof(BenefitListDto))]
        [ProducesResponseType(statusCode: StatusCodes.Status404NotFound)]
        ...
        public async Task<IActionResult> Create([FromForm] BenefitCreateDto benefit)
        {
            try
            {
                await _service.CreateAsync(benefit);
                return Ok();
            }
            catch (ArgumentNullException ex)
            {
        ...
        public async Task<IActionResult> Update([FromRoute] int id, [FromForm] BenefitUpdateDto benefit)
        {
            try
            {
                await _service.UpdateAsync(id, benefit);
                return Ok();
            }
            catch (ArgumentNullException ex)
            {
